...
type StoreManager interface {
	...
	Add(ctx context.Context, userID string, content string, topics []string, metadata map[string]any) error
	// Get retrieves a memory entry by its ID. It should return an error if the entry does not exist.
	Get(ctx context.Context, userID string, memoryID string) (*memory.MemoryEntry, error)
	...
	List(ctx context.Context, userID string, limit int) ([]*memory.MemoryEntry, error)
}
